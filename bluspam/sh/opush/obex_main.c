 ...
 */
int handle_response(obex_t * handle, char *service, int beep)
{
	int err = -1;
...
int obex_disconnect(obex_t * handle)
{
	...
	if (err)
		return err;
	handle_response(handle, BT_SERVICE,0);
	obex_free(gt);
...
obex_t *__obex_connect(int devid, void *addr, int timeout, int *err)
{
	...
	*err = handle_response(handle, BT_SERVICE,0);
...
int obex_push(int devid, void *addr, char *path, char *remote, int timeout)
{
	...
	err = handle_response(handle, BT_SERVICE,1);
...
int main(int argc, char **argv)
{
	...
	ret = obex_push(devid, argv[i], argv[i + 1], argv[i + 2], timeout);
	if ( ret != 0) {
		BTERROR("Error\n");
		return 2;
	}
